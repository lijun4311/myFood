...
import com.mhs66.enums.PayMethod;
import com.mhs66.utils.Redis;
import lombok.AllArgsConstructor;
import org.apache.commons.lang3.StringUtils;
import org.springframework.web.bind.annotation.*;
import pojo.Orders;
import pojo.bo.MerchantOrdersBO;
import pojo.vo.PaymentInfoVO;
import resource.AliPayResource;
import resource.WxPayResource;
...
@AllArgsConstructor
public class PaymentController implements ILogBase {
    public Redis redis;
    ...
    public WebResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isBlank(merchantOrderId)) {
            return WebResult.errorMsg("参数[orderId]不能为空");
        }
        if (StringUtils.isBlank(merchantUserId)) {
            return WebResult.errorMsg("参数[userId]不能为空");
        }
        if (amount == null || amount < 1) {
            return WebResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
        }
        if (payMethod == null) {
            return WebResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
        }
        if (!payMethod.equals(PayMethod.WEIXIN.type) && !payMethod.equals(PayMethod.ALIPAY.type)) {
            return WebResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
        }
        if (StringUtils.isBlank(returnUrl)) {
            return WebResult.errorMsg("参数[returnUrl]不能为空");
        }
        ...
        try {
            ...
            e.printStackTrace();
            WebResult.errorMsg(e.getMessage());
        }

        if (isSuccess) {
            return WebResult.okMsg("商户订单创建成功！");
        } else {
            return WebResult.errorMsg("商户订单创建失败，请重试...");
        }
    }
...
//	@GetMapping(value="/getWXPayQRCode")
    ...
    public WebResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception {
...
//				String total_fee = "1";	// 测试用 1分钱
            ...
            return WebResult.okData(paymentInfoVO);
        } else {
            return WebResult.errorMsg("该订单不存在，或已经支付");
        }
    }
    ...
    public WebResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception {
        ...
        String out_trade_no = merchantOrderId;
        // 付款金额, 必填 单位元
        String total_amount = String.valueOf(waitPayOrder.getAmount());
//    	String total_amount = "0.01";	// 测试用 1分钱
        ...
        return WebResult.okData(alipayForm);
    }
